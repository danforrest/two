...
import copy
import json
from exchange import Exchange, Order, OrderBook, CoinBalance
...
class BinanceExchange(Exchange):
    ...
    def market_convert_coins(self, coin1, coin2, quantity, alt_min=None):
        ...
        if coin1+coin2 in PAIR_LIST:
            ...
            try:
                ...
                return_order.exchange = 'binance'
                return_order.raw_order = order
                print('binance create order details: {}'.format(order))
    ...
    def market_sell_coins(self, coin1, coin2, quantity):
        ...
        if coin1+coin2 in PAIR_LIST:
            # sell
            pair = coin1+coin2
            # round down to the nearest number of digits accepted by binance
            adjusted_quantity = round(quantity - (0.5*self.MIN_AMOUNT[pair]), self.QUANTITY_PRECISION[pair])
            if adjusted_quantity > quantity:
                print('Something went wrong.  Binance sell quantity did not round down.')
    ...
    def market_buy_coins(self, coin1, coin2, quantity):
        if coin1+coin2 in PAIR_LIST:
            ...
            adjusted_quantity = round(quantity - (0.5*self.MIN_AMOUNT[pair]), self.QUANTITY_PRECISION[pair])
            if adjusted_quantity > quantity:
                print('Something went wrong.  Binance buy quantity did not round down.')
                print('Quantity: {} Adjusted: {} Precision: {} Min: {} Before: {}'.format(quantity,
                                                                                          adjusted_quantity,
                                                                                          self.QUANTITY_PRECISION[pair],
                                                                                          self.MIN_AMOUNT[pair],
            ...
            try:
                ...
                return_order.id = order['orderId']
                return_order.pair = pair
                return_order.reserve_coin = coin2
                return_order.timestamp_placed = datetime.utcnow()
                return_order.direction = 'BUY'
                return_order.status = Order.PENDING
                return_order.exchange = 'binance'
    ...
    def update_order(self, order):
        print('binance update order input: {}'.format(order.__dict__))
        if order is None:
            return None
        if order.status == Order.FILLED:
            ...
            return None
        if order.raw_order['status'] == 'FILLED':
            # for sub in order.raw_order['fills']:
            #     trade_order = self.client.get_my_trades(symbol=order.pair,
            #                                             fromId=sub['tradeId'])
            #     print('binance get trade details: {}'.format(trade_order))
            order.id = order.raw_order['orderId']
            order.alt_id = order.raw_order['clientOrderId']
            order.timestamp = float(order.raw_order['transactTime'])
            order.price = float(order.raw_order['price'])
            order.original_price = float(order.raw_order['price'])
            order.quantity = float(order.raw_order['origQty'])
            order.executed_quantity = float(order.raw_order['executedQty'])
            order.fee = {}
            order.status = Order.FILLED
            if order.timestamp_cleared == 0:
                order.timestamp_cleared = datetime.utcnow()
            if 'fills' not in order.raw_order:
                order.sub_orders = []
            else:
                # TODO: need to make this generic
                order.sub_orders = []
                average_price = 0.0
                for sub_order in order.raw_order['fills']:
                    new_sub_order = Order()
                    new_sub_order.price = float(sub_order['price'])
                    new_sub_order.quantity = float(sub_order['qty'])
                    average_price += new_sub_order.price * new_sub_order.quantity
                    new_sub_order.fee = {sub_order['commissionAsset']: float(sub_order['commission'])}
                    if sub_order['commissionAsset'] not in order.fee:
                        order.fee[sub_order['commissionAsset']] = 0.0
                    order.fee[sub_order['commissionAsset']] += float(sub_order['commission'])
                    order.sub_orders.append(new_sub_order)
                order.price = average_price / order.quantity
            print('updated binance order: {}'.format(order.__dict__))
            return order
        elif order.raw_order['status'] == 'CANCELED':
            order.status = Order.CANCELED
            return order
 #       while order.status != 'FILLED':
        # update the order
        binance_order = self.client.get_order(symbol=order.pair,
                                              orderId=order.id)
        print('binance get order details: {}'.format(binance_order))
        trade_order = self.client.get_my_trades(symbol=order.pair,
                                                fromId=binance_order['tradeId'])
        print('binance get trade details: {}'.format(trade_order))
        if binance_order['status'] == 'FILLED':
            order.id = binance_order['orderId']
            order.alt_id = binance_order['clientOrderId']
            order.timestamp = float(binance_order['time'])
            order.price = float(binance_order['price'])
            order.original_price = float(binance_order['price'])
            order.quantity = float(binance_order['origQty'])
            order.fee = {}
            order.status = Order.FILLED
            if order.timestamp_cleared == 0:
                order.timestamp_cleared = datetime.utcnow()
            if 'fills' not in binance_order:
                order.sub_orders = []
            else:
                # TODO: need to make this generic
                order.sub_orders = []
                for sub_order in binance_order['fills']:
                    new_sub_order = Order()
                    new_sub_order.price = float(sub_order['price'])
                    new_sub_order.quantity = float(sub_order['qty'])
                    new_sub_order.fee = {sub_order['commissionAsset']: float(sub_order['commission'])}
    ...
    def query_coin_balances(self):
        ...
        for asset in result['balances']:
            if asset['asset'] in COIN_LIST or asset['asset'] == BNB:
    ...
    # def update_transaction_log_legacy(self, transaction_list):
    ...
    def refill_bnb(self, start_balance, all_transactions):
        ...
        if BNB not in start_value:
            start_value[BNB] = start_balance[BNB] * self.raw_order_book[BNBUSDT].ask

        if start_value[BNB] < 0.001 * total:
            print('Top off BNB')
            order = self.client.order_market_buy(symbol=BNBUSDT, quantity=2.00, newOrderRespType='FULL')
            bnb_order = Order()
            bnb_order.id = order['orderId']
            bnb_order.exchange = 'binance'
            bnb_order.pair = BNB+USDT
            bnb_order.direction = 'BUY'
            bnb_order.alt_id = order['clientOrderId']
            bnb_order.time = int(order['transactTime'])
            bnb_order.price = float(order['price'])
            bnb_order.original_price = float(order['price'])
            bnb_order.quantity = float(order['origQty'])
            bnb_order.executed_quantity = float(order['executedQty'])
            bnb_order.fee = {}
            bnb_order.status = Order.FILLED
            if 'fills' not in order:
                bnb_order.sub_orders = []
            else:
                # TODO: need to make this generic
                bnb_order.sub_orders = []
                average_price = 0.0
                for sub_order in order['fills']:
                    new_sub_order = Order()
                    new_sub_order.price = float(sub_order['price'])
                    new_sub_order.quantity = float(sub_order['qty'])
                    average_price += new_sub_order.price * new_sub_order.quantity
                    new_sub_order.fee = {sub_order['commissionAsset']: float(sub_order['commission'])}
                    bnb_order.sub_orders.append(new_sub_order)
                bnb_order.price = average_price / bnb_order.quantity
            # while 'status' not in order or order['status'] != 'FILLED':
            #     time.sleep(0.5)
            #     order = self.client.get_order(symbol=BNBUSDT, orderId=order['orderId'])
            bnb_order.memo = 'FEE_PAYMENT'
            all_transactions.append(bnb_order)
