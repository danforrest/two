...
class KucoinExchange(Exchange):
    ...
    def cancel_order(self, order):
        ...
        try:
            self.client.cancel_order(symbol=self.local_name[order['symbol']], order_id=order['orderId'])
    ...
    def query_coin_balances(self):
        ...
        for asset in result:
            if asset['coinType'] in COIN_LIST:
                self.balance_book[asset['coinType']].free = asset['balance']
                if asset['freezeBalance'] > 0:
                    self.balance_book[asset['coinType']].locked = asset['freezeBalance']
