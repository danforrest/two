...
def check_arbitrage():
    ...
    if start_bnb_value < 0.001 * (start_coin1_value+start_coin2_value+start_coin3_value):
        ...
        while 'status' not in order or order['status'] != 'FILLED':
            time.sleep(0.5)
            order = client.get_order(symbol='BNBUSDT', orderId=order['orderId'])
    ...
    all_time_high = max(all_time_high, end_total_value)
    if end_total_value < 0.60 * all_time_high and final_return < 0.0:
        exception_logger.error('Time: ' + datetime.utcnow().isoformat())
        exception_logger.error('Total value has dropped below 60% of all time high and lost on a trade')
        exception_logger.error('All Time High: ' + str(all_time_high) + 'Current Value: ' + str(end_total_value))
        print('Total value has dropped below 60% of all time high and lost on a trade')
        print('All Time High: ', all_time_high, 'Current Value: ', end_total_value)
        # sys.exit(-1)
