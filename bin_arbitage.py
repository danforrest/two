...
class BinanceArbitrage():
    ...
    def convert_coins(self, coin1, coin2, quantity):
        ...
        if coin1+coin2 in [self.PAIR1, self.PAIR2, self.PAIR3]:
            ...
            if self.trade_order_book[pair].ask * adjusted_quantity < self.MIN_NOTIONAL[pair]:
                print('Value under min notional')
            ...
            if self.trade_order_book[pair].bid * adjusted_quantity < self.MIN_NOTIONAL[pair]:
                print('Value under min notional')
                print('Price: ', price, 'quantity: ', adjusted_quantity, 'min: ', self.MIN_NOTIONAL[pair])
                return None
            try:
    ...
    def start_logging(self):
        ...
        order_log_file_handler = logging.FileHandler('logs\order_tracker_%s.log' % self.log_start_time.isoformat())
        order_log_file_handler.setLevel(logging.INFO)
        exception_log_file_handler = logging.FileHandler('logs\exception_tracker_%s.log' % self.log_start_time.isoformat())
    ...
    def cancel_all_orders(self):
        orders = self.client.get_all_orders(symbol=self.PAIR1)
        for order in orders:
            self.client.cancel_order(symbol=self.PAIR1, orderId=order['orderId'])
        orders = self.client.get_all_orders(symbol=self.PAIR2)
        for order in orders:
            self.client.cancel_order(symbol=self.PAIR2, orderId=order['orderId'])
        orders = self.client.get_all_orders(symbol=self.PAIR3)
        for order in orders:
    ...
    def check_arbitrage(self):
        ...
        if start_bnb_value < 0.001 * (start_coin1_value+start_coin2_value+start_coin3_value):
            order = self.client.order_market_buy(symbol='BNBUSDT', quantity=2.00)
        ...
        if found_order:
            ...
            while (pair1_order['status'] not in ['FILLED', 'None']
                ...
                if pair1_order != self.EMPTY_ORDER and pair1_order['status'] != 'FILLED':
                    pair1_order = self.client.get_order(symbol=self.PAIR1, orderId=pair1_order['orderId'])
                ...
                if pair2_order != self.EMPTY_ORDER and pair2_order['status'] != 'FILLED':
                    pair2_order = self.client.get_order(symbol=self.PAIR2, orderId=pair2_order['orderId'])
                ...
                if pair3_order != self.EMPTY_ORDER and pair3_order['status'] != 'FILLED':
                    pair3_order = self.client.get_order(symbol=self.PAIR3, orderId=pair3_order['orderId'])
        ...
        if found_order:
            ...
            if pair1_order != self.EMPTY_ORDER:
                pair1_check_order = self.client.get_order(symbol=self.PAIR1, orderId=pair1_order['orderId'])
            ...
            if pair2_order != self.EMPTY_ORDER:
                pair2_check_order = self.client.get_order(symbol=self.PAIR2, orderId=pair2_order['orderId'])
            ...
            if pair3_order != self.EMPTY_ORDER:
                pair3_check_order = self.client.get_order(symbol=self.PAIR3, orderId=pair3_order['orderId'])
            ...
            log_list = [order_start_time, order_end_time, self.PAIR1, self.PAIR2, self.PAIR3, final_return,
