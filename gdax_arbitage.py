...
class GdaxWebsocketClient(gdax.WebsocketClient):
    ...
    def __init__(self, order_book=order_book):
        super().__init__()
        self.order_book = order_book

    def on_open(self):
        self.url = "wss://ws-feed.gdax.com/"
        self.products = [GdaxArbitrage.PAIR1, GdaxArbitrage.PAIR2, GdaxArbitrage.PAIR3]
        self.auth = True
        self.auth_key = GdaxArbitrage.key
        self.auth_b64secret = GdaxArbitrage.b64secret
        self.auth_passphrase = GdaxArbitrage.passphrase
        self.channels = ['ticker', 'user']

    def on_message(self, msg):
        if 'type' in msg and msg['type'] != 'ticker':
            print("Message:", msg)
        if 'type' in msg and msg['type'] not in ['ticker', 'user']:
            return
        if 'product_id' in msg and msg['product_id'] in self.order_book:
            self.order_book[msg['product_id']].bid = float(msg['best_bid'])
            self.order_book[msg['product_id']].ask = float(msg['best_ask'])
        else:
            return
...
class GdaxArbitrage():
    ...
    passphrase = '4nb01afawgm'
    key = '8dca34fe64bfca9ff6466721d433b2a7'
    b64secret = '2kUZwnOSZhaa2GX+hqDQDQiKPq9RMOwckF/GypKSh9xxkyt+lj0H2Wnv5v7VT3Hc+3zx3y3xRgAPHtwbEqGbIw=='
    ...
    TICK = {'ETH-BTC': 0.00001,
            'BTC-USD': 0.01,
            'ETH-USD': 0.01}
    PRICE_PRECISION = {'ETH-BTC': 5,
                       'BTC-USD': 2,
                       'ETH-USD': 2}
    PRICE_FORMAT = {'ETH-BTC': '%.5f',
                    'BTC-USD': '%.2f',
                    'ETH-USD': '%.2f'}
    QUANTITY_PRECISION = {'ETH-BTC': 8,
                          'BTC-USD': 8,
                          'ETH-USD': 8}
    SPREAD_THRESHOLD = {'ETH-BTC': 0.75,
                        'BTC-USD': 0.75,
                        'ETH-USD': 0.75}
    MIN_AMOUNT = {'ETH-BTC': 0.01,
                  'BTC-USD': 0.001,
                  'ETH-USD': 0.01}
    # MIN_NOTIONAL = {'ETH-BTC': 0.001,
    #                 'BTC-USD': 1.0,
    #                 'ETH-USD': 20.0}
    ...
    raw_order_book = {'ETH-BTC': OrderBook(),
                      'BTC-USD': OrderBook(),
                      'ETH-USD': OrderBook()}
    ...
    def convert_coins(self, coin1, coin2, quantity):
        ...
        if coin1+'-'+coin2 in [self.PAIR1, self.PAIR2, self.PAIR3]:
            ...
            if adjusted_quantity < self.MIN_AMOUNT[pair]:
                ...
                return None
            try:
                order = self.auth_client.sell(product_id=pair,
                                              price=price,
            ...
            if adjusted_quantity < self.MIN_AMOUNT[pair]:
                ...
                return None
            try:
                order = self.auth_client.buy(product_id=pair,
                                             price=price,
                                             size=str(round(adjusted_quantity,
                                                            self.QUANTITY_PRECISION[pair])),
                                             post_only=True)
    ...
    def start_logging(self):
        ...
        self.order_logger = logging.getLogger('gdax_order_tracker')
        self.order_logger.setLevel(logging.DEBUG)
        self.exception_logger = logging.getLogger('gdax_exception_tracker')
        self.exception_logger.setLevel(logging.DEBUG)

        order_log_file_handler = logging.FileHandler('logs\gdax_order_tracker_%s.log' % self.log_start_time.isoformat())
        order_log_file_handler.setLevel(logging.INFO)
        exception_log_file_handler = logging.FileHandler('logs\gdax_exception_tracker_%s.log' % self.log_start_time.isoformat())
    ...
    def launch_socket_listeners(self):
        self.websocket_client = GdaxWebsocketClient(self.raw_order_book)
        ...
        while self.raw_order_book['ETH-BTC'].bid == 0 or \
                self.raw_order_book['BTC-USD'].bid == 0 or \
                self.raw_order_book['ETH-USD'].bid == 0:
            time.sleep(1)
    ...
    def cancel_all_orders(self):
        self.auth_client.cancel_all(product=self.PAIR1)
        self.auth_client.cancel_all(product=self.PAIR2)
        self.auth_client.cancel_all(product=self.PAIR3)
    ...
    def check_arbitrage(self):
        ...
        if self.COIN1+'-USD' in self.raw_order_book:
            coin1_price = self.raw_order_book[self.COIN1+'-USD'].ask
        if self.COIN2+'-USD' in self.raw_order_book:
            coin2_price = self.raw_order_book[self.COIN2+'-USD'].ask
        if self.COIN3+'-USD' in self.raw_order_book:
            coin3_price = self.raw_order_book[self.COIN3+'-USD'].ask
        ...
        if found_order:
            ...
            while (pair1_order['status'] not in ['done', 'None']
                ...
                if pair1_order != self.EMPTY_ORDER and pair1_order['status'] != 'done':
                    pair1_order = self.auth_client.get_order(pair1_order['id'])
                ...
                if pair2_order != self.EMPTY_ORDER and pair2_order['status'] != 'done':
                    pair2_order = self.auth_client.get_order(pair2_order['id'])
                ...
                if pair3_order != self.EMPTY_ORDER and pair3_order['status'] != 'done':
                    pair3_order = self.auth_client.get_order(pair3_order['id'])
        ...
        if found_order:
            ...
            if pair1_order != self.EMPTY_ORDER:
                pair1_check_order = self.auth_client.get_order(pair1_order['id'])
            ...
            if pair2_order != self.EMPTY_ORDER:
                pair2_check_order = self.auth_client.get_order(pair2_order['id'])
            ...
            if pair3_order != self.EMPTY_ORDER:
                pair3_check_order = self.auth_client.get_order(pair3_order['id'])
            ...
            log_list = ['gdax',order_start_time, order_end_time, self.PAIR1, self.PAIR2, self.PAIR3, final_return,
        ...
        while True:
            try:
                ...
                if exception_count >= 3:
                    ...
                    raise e
                time.sleep(3)
                self.auth_client = gdax.AuthenticatedClient(self.key,
                                                            self.b64secret,
                                                            self.passphrase)
                self.query_coin_balances()
                self.launch_socket_listeners()
...
if __name__== "__main__":
    binance_arbitrage = GdaxArbitrage()
